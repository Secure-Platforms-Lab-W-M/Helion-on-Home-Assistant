...
def main() -> int:
    """Start Home Assistant."""
    # call to appdaemon as subprocess, assuming it is installed on your machine
    # if not, install with command 'sudo pip3 install appdaemon'
    args = get_arguments()
    appdaemon_dir = os.path.abspath(os.path.join(os.getcwd(), args.config, "appdaemon"))
    if (os.path.exists(appdaemon_dir) == False):
        print("You have not set up the appdaemon directory in your configuration, please do so before using AppDaemon!")
    else:
        cmd = ['appdaemon', '-c', appdaemon_dir]
