...
class Ngram(object):
  ...
  def _ngram_backoff(self, flavor, history, n):
    ...
    f = lambda x: list(zip(*[(k, 10 ** v) for k, v in x]))
    ...
    if n == 1: # ...where unigrams serve as the base case
      ...
      tokens, probabilities = f(list(self._ngram_buckets[0].items()))
  ...
  def _ngram_interpolate(self, flavor, history, n):
    ...
    for order in range(n, 0, -1): # Start at the highest order and walk down
      ...
      if order == 1:
        ...
        for token, probability in list(self._ngram_buckets[0].items()):
    ...
    tokens, probabilities = list(zip(*[(k, v) for k, v in list(self._distribution.items())]))
